...
urlpatterns = [
    ...
    path('map_mark/', StoreListView.as_view(), name='map_mark'),  # 지도 마커용 가맹점 리스트 반환
    path('category_map_mark/', CategoryListView.as_view(), name='category_map_mark'),  # 지도 마커용 카테고리별 가맹점 리스트 반환
    path('search_map_mark/', SearchListView.as_view(), name='search_map_mark'),  # 지도 마커용 검색어별 가맹점 리스트 반환
]
